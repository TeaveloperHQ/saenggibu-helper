...
- **행 선택**: 행번호 클릭(Ctrl/Shift로 여러 개). 생성은 선택한 행에 채운다.
- **표현 버리기**(셀 우클릭): 내용 칸을 우클릭 → *이 표현 버리기* — 그 문장을 지우고 다음 생성부터 비슷한 표현을 피한다.
- **행 높이·열 너비**: 행번호·머리글 경계 드래그(행별 개별 조절, 학급 탭별 저장). 내용 열은 **남는 폭을 자동으로 채움** —
  직접 조절하면 그 폭을 유지하고, 머리글 우클릭 → *창 너비에 맞추기*로 다시 자동 채움. 내용은 **열 너비에 맞춰 줄바꿈 + 행높이 자동맞춤**.
- **복사/붙여넣기**(Ctrl+C / Ctrl+V, TSV — 현재 선택 위치에 붙임), **Delete**(선택한 셀만 비움 · 행번호로 선택했으면 행 전체),
  **Ctrl+Z / Ctrl+Y**(되돌리기 / 다시 실행), **Ctrl+휠**(화면 확대/축소).
- **좌상단 코너 전체선택**: 학번=숫자·이름=한글 패턴으로 **학적 있는 행만** 능동 선택.
- **생성 대상 열**: 모든 열을 표시하고 기본은 **영역 탭 이름과 가장 가까운 열 → 이름은 있는데 비어 있는 열** 순으로 자동 선택. 최종 선택은 교사가 한다.
- **찾기·바꾸기**: **현재 시트** 또는 **전체 학급** 범위로 내용 열 일괄 치환.
- **엑셀 가져오기/내보내기**(.xlsx), **맞춤법 검사**, **⛶ 전체화면**.
- **저장 = 파일 저장 + 학습**: 내용 열 중 **새 생기부 문장만** 학습 — 규칙(문장 꼴) → 이미 학습·버린 문장 제외 → **언어 모델이 생기부 문장인지 판별** 순.
  숫자·학기·이름·메모 조각은 학습하지 않는다. 판별은 백그라운드로 돌아 저장은 바로 끝난다.
