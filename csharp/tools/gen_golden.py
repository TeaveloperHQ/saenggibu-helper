...
from app.variation import combine_variants, split_sentences  # noqa: E402
...
COMBINE_CASES = [
    ([["맡은 역할을 성실히 수행함", "맡은 일을 끝까지 해냄"],
      ["친구를 배려하는 태도를 보임", "또래를 돕는 자세를 지님"]], 6),
    ([["탐구하는 태도를 보임"]], 5),
    ([["실험을 설계함", "실험을 직접 구상함"],
      ["결과를 그래프로 정리함", "자료를 표로 나타냄"],
      ["원인을 토의함"]], 10),
]


def pyrandom_cases():
    r = random.Random(42); grb = [r.getrandbits(32) for _ in range(8)]
    r = random.Random(42); lst = list(range(12)); r.shuffle(lst)
    r = random.Random(42); ch = [r.choice(["a", "b", "c", "d", "e"]) for _ in range(8)]
    r = random.Random(42); rnd = [f"{r.random():.15f}" for _ in range(4)]
    r = random.Random(123); grb2 = [r.getrandbits(32) for _ in range(4)]
    return {"getrandbits32": grb, "shuffle12": lst, "choice5": ch,
            "random15": rnd, "getrandbits32_s123": grb2}
...
def main() -> int:
    golden = {
        ...
        "pyrandom": pyrandom_cases(),
        "combine_variants": [
            {"groups": g, "n": n, "out": combine_variants([list(x) for x in g], n)}
            for g, n in COMBINE_CASES
        ],
    }
